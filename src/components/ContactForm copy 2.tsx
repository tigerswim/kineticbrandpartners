...
// ───────────────── date helpers ─────────────────
const parseDate = (str = '') => {
  if (!str.includes('-')) return { y: '', m: '' }
  const [y, m] = str.split('-')
  return { y, m }
}
const combineDate = (m: string, y: string) => (m && y ? `${y}-${m}` : '')
...
export default function ContactForm({ contact, onSuccess, onCancel }: ContactFormProps) {
  ...
  const updateExperienceDate = (
    ...
    const dateString = combineDate(month.trim(), year.trim())
    console.log(
      `Updating ${dateType} for experience ${index}: month=${month}, year=${year}, dateString=${dateString}`
    )
    setExperience(exp =>
      exp.map((e, i) => (i === index ? { ...e, [dateType]: dateString } : e))
    )
  }
  ...
  const updateEducationDate = (index: number, month: string, year: string) => {
    const dateString = combineDate(month.trim(), year.trim())
    console.log(
      `Updating education ${index}: month=${month}, year=${year}, dateString=${dateString}`
    )
    setEducation(edu =>
      edu.map((e, i) => (i === index ? { ...e, year: dateString } : e))
    )
  }
