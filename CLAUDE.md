...
2. **Candidate pages** — `/sbd`, `/gc`, `/ups`, `/ws`, `/keh`, `/locumtenens`, `/finsync`, `/earnix`, `/ashley`, `/calendly`. These keep the **legacy Kinetic Flow** styles in `globals.css` plus per-page brand overrides in `src/app/<page>/page.css`. Do not edit `kbp-main.css` for candidate-page changes.
